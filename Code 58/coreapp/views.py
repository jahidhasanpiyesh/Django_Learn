...
from .forms import UserForm, UserChange, UserAdminChange
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            if request.user.is_superuser == True:
                fm = UserAdminChange(request.POST, instance=request.user)
                users = User.objects.all()
            else:
                # inharetance forms.py class 
                fm = UserChange(request.POST, instance = request.user)
                users = None
            if fm.is_valid():
                messages.success(request,'Your Profile Data Update Successfully !!')
                fm.save()
        else:
            if request.user.is_superuser == True :
                fm = UserAdminChange(instance=request.user)
                users = User.objects.all()
            else:
                fm = UserChange(instance=request.user)
                users = None
        return render(request,'coreapp/profile.html',{'name':request.user.username,'form':fm, 'users':users})
    else:
        return HttpResponseRedirect('/login/')
...
def userinfo(request, id):
    if request.user.is_authenticated:
        ...
        fm = UserAdminChange(instance=pi)
        return render(request,'coreapp/userinfo.html',{'form':fm})
    else:
        return HttpResponseRedirect('/login/')